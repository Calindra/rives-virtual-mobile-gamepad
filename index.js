...
async function rivemuRecord(cartridge, incard, entropy, args) {
    ...
    Module.ccall(
        ...
        [cartridgeBuf, cartridge.length, incardBuf, incard.length, entropy, args]
    );
...
async function rivemuReplay(tape, cartridge, incard, entropy, args) {
    ...
    Module.ccall(
        ...
        [cartridgeBuf, cartridge.length, incardBuf, incard.length, entropy, args, tapeBuf, tape.length]
    );
    Module._free(cartridgeBuf);
    Module._free(incardBuf);
...
function rivemu_on_begin(width, height, target_fps, total_frames, info_data) {
    ...
    resolutionElem.textContent = width + 'x' + height + '@' + target_fps + ' -> ' + canvasElem.width + 'x' + canvasElem.height + '@60';
